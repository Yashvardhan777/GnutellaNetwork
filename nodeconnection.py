...
class NodeConnection(threading.Thread):
    def __init__(self, main_node, soc, id, host, port):
    ...
    def send(self, data, encoding_type='utf-8'):
        ...
        if isinstance(data, str):
            try:
                self.soc.sendall( data.encode(encoding_type) + self.EOT_CHAR )

            except Exception as e:
                print("nodeconnection send: Error sending data to node: " + str(e))
                self.stop()
